...
def plot_step(tstep):
  ...
  rho_step = np.transpose(np.sum(rho[:,:,:,tstep], axis=2)/nz)
  mx_step  = np.transpose(np.sum( mx[:,:,:,tstep], axis=2)/nz)
  my_step  = np.transpose(np.sum( my[:,:,:,tstep], axis=2)/nz)
  et_step  = np.transpose(np.sum( et[:,:,:,tstep], axis=2)/nz)
  U = mx_step/rho_step
  V = my_step/rho_step
  ...
  # stream plots
  numfigs += 1
  fig = plt.figure(numfigs,figsize=(5,4))
  lw = speed / speed.max()
